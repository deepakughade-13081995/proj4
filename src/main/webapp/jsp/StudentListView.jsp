...
<tr>
<td align="center">
<label>FirstName:</label><input type="text" name="firstName" value="<%=ServletUtility.getParameter("firstName",request)%>">
 
<label>LastName:</label><input type="text" name="lastName" value="<%=ServletUtility.getParameter("lastName",request)%>">

<label>Email_id:</label><input type="text" name="email" value="<%=ServletUtility.getParameter("email",request)%>">
...
<tr>
                ...
                %>
                  ...
                  <tr>
                    ...
                    <td align="center"><%=bean1.getLastName()%></td>
                     <td align="center"><%=bean1.getCollageName()%></td>
                     <td align="center"><%=bean1.getAddress()%></td>
                    <td align="center"><%=bean1.getDob()%></td>
                    <td align="center"><%=bean1.getMobileNo()%></td>
                    <td align="center"><%=bean1.getEmail()%></td>
                    <%if(userBean1.getRoleId()==4 ||userBean1.getRoleId()==3 ||userBean1.getRoleId()==2){ %>
                    
                    <%}else{ %>
                     <td align="center"><a href="StudentCtl?id=<%=bean1.getId()%>">Edit</a></td>
                     <%} %>
            </tr>
                <%
                    }
 ...
 <table width="100%">
                <tr>
                 <%if(pageNo==1){ %>   <td><input type="submit" name="operation" disabled="disabled"
                      ...
                      <% } else{ %>  <td><input type="submit" name="operation"
                        ...
                        <%} %>
                   <%if(userBean1.getRoleId()==2||userBean1.getRoleId()==3||userBean1.getRoleId()==4){ %>
                    <td><input type="submit"
                        name="operation" disabled="disabled" value="<%=StudentListCtl.OP_DELETE%>"></td>
     ...
     <%} if(list1.size()==0){%>
        <table>
        <tr>
        <td><input type="submit" name="operation" value="<%=StudentListCtl.OP_BACK%>"></td></tr>
        </table>
              <%} %>  
