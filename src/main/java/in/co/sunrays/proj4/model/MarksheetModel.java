...
public class MarksheetModel {
 ...
 public long add(MarksheetBean bean) throws ApplicationException,
        ...
        System.out.println("duplicateMarksheet"+duplicateMarksheet);
        int pk = 0;
        ...
        try {
            ...
            pstmt.setInt(6, bean.getChemestry());
    ...
    public List search(MarksheetBean bean, int pageNo, int pageSize)
        ...
        if (bean != null) {
            ...
            if (bean.getChemestry() != null && bean.getChemestry() > 0) {
                sql.append(" AND chemistry = " + bean.getChemestry());
            }
    ...
    public List getMeritList(int pageNo, int pageSize)
        ...
        ArrayList list = new ArrayList();
        StringBuffer sql = new StringBuffer(
                "SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMISTRY`, `MATHS` , (PHYSICS + CHEMISTRY + MATHS) as total from `ST_MARKSHEET` where not(physics<33 or chemistry<33 or maths<33) order by total desc");
        // if page size is greater than zero then apply pagination
        if (pageSize > 0) {
            // Calculate start record index
            pageNo = (pageNo - 1) * pageSize;
